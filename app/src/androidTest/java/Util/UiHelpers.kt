...
fun ViewInteraction.waitFor(millis: Long): ViewInteraction = this.perform(waitForMillis(millis))
...
fun Matcher<View>.first(): ViewInteraction = Espresso.onView(withIndex(this, 0))
fun Matcher<View>.withIndex(index: Int): ViewInteraction = Espresso.onView(withIndex(this, index))
...
fun all(vararg resIdOrTextOrMatcher: Any): Matcher<View> {
    return CoreMatchers.allOf(mapToMatchers(resIdOrTextOrMatcher.toList()))
}

fun any(vararg resIdOrTextOrMatcher: Any): Matcher<View> {
    return CoreMatchers.anyOf(mapToMatchers(resIdOrTextOrMatcher.toList()))
}
